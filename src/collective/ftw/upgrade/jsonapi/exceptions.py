...
class WrongAPIVersion(APIError):
    def __init__(self, requested_version):
        super().__init__(
            'Wrong API version',
            'The API version "{}" is not available.'.format(
                requested_version),
            response_code=404)
...
class MissingParam(APIError):
    def __init__(self, param_name):
        super().__init__(
            'Param missing',
            'The param "{}" is required for this API action.'.format(
                param_name))
...
class ProfileNotFound(APIError):
    def __init__(self, profileid):
        super().__init__(
            'Profile not found',
            f'The profile "{profileid}" is unknown.')
...
class UpgradeNotFoundWrapper(APIError):
    def __init__(self, original_exception):
        ...
        super().__init__(
            'Upgrade not found',
            f'The upgrade "{api_upgrade_id}" is unkown.')
