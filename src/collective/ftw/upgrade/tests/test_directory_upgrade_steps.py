...
class TestDirectoryUpgradeSteps(UpgradeTestCase):
    ...
    def test_installing_profile_sets_version_to_latest_upgrade(self):
        self.package.with_profile(
            Builder("genericsetup profile").with_upgrade(
                Builder("ftw upgrade step").to(datetime(2014, 1, 1))
            )
        )
    ...
    def test_installing_profile_marks_all_upgrade_steps_as_installed_too(self):
        ...
        with self.package_created():
            recorder = getMultiAdapter(
                (self.portal, "the.package:default"), IUpgradeStepRecorder
            )
            self.assertFalse(
    ...
    def test_installing_profile_does_not_mark_upgrade_step_as_installed_on_partial_import(
        self,
    ):
        self.package.with_profile(
            Builder("genericsetup profile").with_upgrade(
        ...
        with self.package_created():
            ...
            self.portal_setup.runImportStepFromProfile(
                "profile-the.package:default", "typeinfo"
            )
    ...
    def test_running_upgrade_steps_marks_them_as_installed(self):
        ...
        with self.package_created():
            ...
            self.assertTrue(
                recorder.is_installed("20110101000000"),
                "Expected upgrade steps to be marked as installed" " after upgrading.",
            )
