...
class IImportProfileUpgradeStep(Interface):
    ...
    title = zope.schema.TextLine(title="Title", required=True)

    description = zope.schema.TextLine(title="Upgrade step description", required=False)

    profile = zope.schema.TextLine(title="GenericSetup profile id", required=True)

    source = zope.schema.ASCII(title="Source version", required=True)

    destination = zope.schema.ASCII(title="Destination version", required=True)

    directory = Path(title="Path", required=True)
