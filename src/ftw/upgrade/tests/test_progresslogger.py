...
class TestProgressLogger(TestCase):
    ...
    def test_succeeding_logging(self):
        with ProgressLogger("Foo", 5, logger=self.logger, timeout=0.03) as step:
            for i in range(5):
                step()
                sleep(0.0151)
    ...
    def test_failing_logging(self):
        ...
        with self.assertRaises(ValueError):
            ...
            with ProgressLogger(
                "Bar", data, logger=self.logger, timeout=timeout
            ) as step:
                for i in data:
                    if i == 3:
    ...
    def test_accepts_iterable_object(self):
        ...
        self.assertEqual(
            ["STARTING Foo", "1 of 5 (20%): Foo", "DONE Foo"], self.read_log()
        )
    ...
    def test_current_item_is_printed_when_logger_exits_unexpectedly(self):
        ...
        self.assertEqual(
            ["STARTING Foo", "1 of 5 (20%): Foo", "FAILED Foo (GeneratorExit: ) at 4"],
            self.read_log(),
        )
