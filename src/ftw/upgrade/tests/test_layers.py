from ftw.upgrade.tests.layers import COMPONENT_REGISTRY_ISOLATION
from ftw.upgrade.tests.layers import ConsoleScriptLayer
from ftw.upgrade.tests.layers import TEMP_DIRECTORY
...
COMPONENT_REGISTRY_ISOLATION_INTEGRATION = IntegrationTesting(
    ...
    name="ftw.upgrade:test_component_registry_isolation_layer:integration",
)
...
class TestComponentRegistryIsolationLayer(TestCase):
    ...
    def test_loading_zcml_string(self):
        self.assertFalse(
            self.profile_exists(), "Component registry is not isolated per test."
        )
        self.layer["load_zcml_string"](GENERICSETUP_PROFILE_ZCML)
        self.assertTrue(self.profile_exists(), "Failed to register profile.")
    ...
    def profile_exists(self):
        ...
        try:
            portal_setup.getProfileInfo("ftw.upgrade:default")
        except KeyError:
            return False
        else:
            return True
...
CONSOLE_SCRIPT_TESTING = ConsoleScriptLayer("ftw.upgrade")
...
class TestConsoleScriptLayer(TestCase):
    ...
    def test_buildout_script_is_generated(self):
        self.assertTrue(
            self.layer["root_path"].joinpath("bin", "buildout").is_file(),
            "bin/buildout script was not generated.",
        )

    def test_buildout_config_is_generated(self):
        self.assertTrue(
            self.layer["root_path"].joinpath("buildout.cfg").is_file(),
    ...
    def test_buildout_directory_is_isolated_for_each_test(self):
        ...
        self.assertFalse(
            path.exists(),
            "Additionally created files in buildout"
    ...
    def test_python_script_with_environment_is_generated(self):
        code = "from ftw.upgrade.tests import layers; print(layers.__name__)"
        exitcode, output = self.layer["execute_script"](f'py -c "{code}"')
        self.assertEqual("ftw.upgrade.tests.layers", output.splitlines()[0])
